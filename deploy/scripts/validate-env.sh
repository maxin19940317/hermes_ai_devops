...
for key in POSTGRES_ADMIN_PASSWORD RUNTIME_DB_PASSWORD GITLAB_TOKEN TRIGGER_WEBHOOK_SECRET MINIO_ROOT_PASSWORD GRAFANA_ADMIN_PASSWORD; do
  eval "value=\${$key:-}"
  test -n "$value" || { echo "ERROR: $key is empty" >&2; exit 1; }
done

for key in POSTGRES_IMAGE TEMPORAL_IMAGE TEMPORAL_UI_IMAGE GO_IMAGE RUNTIME_BASE_IMAGE MINIO_IMAGE MINIO_MC_IMAGE GRAFANA_IMAGE; do
  eval "value=\${$key:-}"
  case "$value" in
    *@sha256:*) ;;
...
for spec in "TRIGGER_HOST_PORT 18090" "WORKER_CALLBACKS_HOST_PORT 18091" "TEMPORAL_UI_HOST_PORT 18080" "MINIO_HOST_PORT 9000" "MINIO_CONSOLE_PORT 9001" "GRAFANA_HOST_PORT 13000"; do
  var=${spec% *}
  def=${spec#* }
  eval "port=\${$var:-$def}"
