...
# Secrets travel in 0600 curl config files, never as argv visible in ps(1).
gitlab_curl_cfg=$(mktemp)
webhook_curl_cfg=$(mktemp)
response_file=$(mktemp)
printf 'header = "%s: %s"\n' "$GITLAB_TOKEN_HEADER" "$GITLAB_TOKEN" >"$gitlab_curl_cfg"
printf 'header = "X-Gitlab-Token: %s"\n' "$TRIGGER_WEBHOOK_SECRET" >"$webhook_curl_cfg"
...
pipeline=$(curl -fsS -K "$gitlab_curl_cfg" \
...
second_status=$(curl -sS -o "$response_file" -w '%{http_code}' \
  ...
  -K "$webhook_curl_cfg" \
  --data-binary "$payload" "$trigger_url/webhooks/gitlab")
if test "$second_status" != 202; then
  echo "ERROR: duplicate webhook returned HTTP $second_status" >&2
  sed -n '1,5p' "$response_file" >&2
  exit 1
fi
jq -e '.started == false' "$response_file" >/dev/null || {
  echo "ERROR: duplicate webhook did not return started:false" >&2
  exit 1
}
