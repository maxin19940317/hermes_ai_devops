...
### 3.1 `df` 空间不足直判 device 的前提与残余风险

上表把"df 成功但空间不足"与"存活复核确认设备不可达"并列为 device 的两条独立证据
来源,理由是"df 已成功执行,数值不足是自足的设备状态证据"——唯一绕过 §5.3 两级
存活复核的分支。但这个前提本身有一处未被上表承认的漏洞:`min_free_storage_mb` 是
manifest 声明的阈值(`ci/gen_manifest.py` 按 `ci/variants.yaml` 的变体映射生成),
不是设备侧读出来的量。

**实际触发条件**:阈值配置漂移(CI 改动误改数值、variants.yaml 映射错配、给某变体
设了一个过高的门槛),会让所有跑该变体的板子在"df 成功但数值不达标"这条路径上持续
判 device——三次后**该 fleet 的每一块板都会被自动隔离**。这与本节 soc mismatch 那
一行是同构的风险模式:配置错误连续触发、误伤健康设备、真实故障(配置本身)反被掩盖。
恢复同样只能靠 §8 记录的人工 `unquarantine`,没有自动降级路径。

**为什么仍然选择直判 device,而不是像 abi/soc mismatch 那样降级为 `none`**:
abi/soc mismatch 比较的是固定的硬件规格——声明的期望值一旦配错,所有设备的比较
结果永远错,没有"设备状态变化后自己恢复"的可能,所以宁可不计。空间不足不同:即便
阈值本身合理,数值本身主要反映设备当下的累积状态(旧任务产物未清理、其它进程占用
存储),清理现场或换一批任务就能恢复,更接近"设备状态的证据"而非"固定规格比较的
证据"。这是本设计**已确认的权衡,不是疏漏**:接受"阈值配置漂移可能导致批量隔离"
这个尾部风险,换取"真实空间不足能被正确记账"这个更常见场景不必绕行复核。

**残余盲区**:如果未来这个尾部风险被认为不可接受,可选的加固方向包括:要求
`availKB > 0`(排除 df 输出本身异常但退出码恰好为 0 的情形)才信任比较结果;或者在
Store 层加一个 fleet 级熔断——不允许把某个变体最后一台可租设备自动隔离掉,强制至少
留一块板供人工排查。本轮不做,留作后续。

**同一路径上一个更小的既有问题**:`df` 的退出码目前未被检查就直接解析 stdout——
"df 已成功执行"这个前提本身没有被强制过。这与 §5.4 第 2 处 `resolveTransport` 的
退出码漏检是同一类问题,规模更小,留作后续小清理,不在本轮范围内。
...
| 上述失败经**存活复核**确认设备不可达 | `device` | 调用失败路径的唯一 device 来源 |
...
### 5.3 存活复核(调用失败升级为 device 的唯一一般性途径)
