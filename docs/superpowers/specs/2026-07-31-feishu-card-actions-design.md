...
2. **失去租约的写方零写入。** 三重 fencing 一旦确立，"失租后还能改点什么"就不再成立——
   ...
   可能覆盖他人刚设的复核窗口。失租的写方**只记日志，一个字段都不碰**。

   写方失去持有权只有两种成因，两种都收敛，**不能只说其中一种**：

   | 成因 | 行的当下状态 | 谁来完成 |
   |---|---|---|
   | 租约到期后被别的 worker 重新 claim | 有新 owner、租约有效 | 新 owner 渲染当前 `desired_revision`；它若也失败，租约到期后再被 claim |
   | §5.4 的 revision 重排**无条件清空了 owner** | **ownerless、`pending`、租约为 NULL** | 此时**没有新持有者**——但该行立即满足 §8.1 的 sweep 谓词（`lease_expires_at IS NULL`），下一轮直接被重新 claim |

   第二种是常态而非例外：动作每推进一次版本就会清一次 owner。因此
