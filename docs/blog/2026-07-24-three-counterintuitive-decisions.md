...
先说清楚两个概念，全文都不翻译。"跑完了没有"是 status——只描述生命周期走到哪一步：排队、下发、运行、超时、完成。"这次算不算通过"是 verdict——是对结果的最终判定：通过、测试失败、性能回归、基础设施错误、无法判定。同样是超时，可能是设备卡死，也可能压根没连上，但都该归到基础设施问题、允许重试——这就是同一个 status 对应不同判断维度的例子。一旦把两者揉进同一个字段，将来就再也拆不开：决定一里的签名分类之所以能生效，前提正是 verdict 里留得下"归到哪一类错误"这个信息，不能被 status 占掉。
...
LLM 在这里第一次正式出场——以"我们决定不让它做什么"的身份登场。它不参与裁决，原因不是调用 LLM 会打乱重放：那次调用只要包成独立一步、结果记进历史，重放时一样只读记录不重新问它，这条路径本身没问题。真正的原因是另外两条：一是 verdict 要经得起三个月后的审计复现，同一个问题问 LLM 两次未必答案一致；二是 LLM 会挂、会超时、会被限流，而已经开始的任务必须能收敛到终态，不能因为一次调用没等到响应就一直悬着。
...
顺序也不是随手排的：签名判断排在超时和用例计数之前——命中 `cpu_fallback` 说明模型压根没跑上 DSP，是模型层面的问题，不该被简单归为超时。签名携带的信息量最大，能直接定位具体原因，理应比"超时了"这种笼统结论优先采信；反过来先判超时，一次因 DSP 不可用而卡到超时的运行，会被草率记成"基础设施超时"，白白丢掉日志里已写明的真实原因。
